...
Specifically, the partial sum is: <img src="https://latex.codecogs.com/svg.image?s_k&space;=&space;\sum_{i=1}^{k}i&space;=&space;\frac{k(k&plus;1)}{2}" title="s_k = \sum_{i=1}^{k}i = \frac{k(k+1)}{2}" />. Hence it is easy to verify that ![formula](https://render.githubusercontent.com/render/math?math=s^*%20\leq%20a_{max}) for the best value of *k*<sup>*</sup>:
