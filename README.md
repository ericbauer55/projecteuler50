...
Specifically, the partial sum is: ![formula](https://render.githubusercontent.com/render/math?math=s_k%20=%20=%20\sum_{j=1}^k%20j%20=%20\frac{k(k+1)}{2}). Hence it is easy to verify that ![formula](https://render.githubusercontent.com/render/math?math=s^*%20\leq%20a_{max}) for the following:
1. ![formula](https://render.githubusercontent.com/render/math?math=s^*=\frac{k^*(k^*+1)}{2}%20\leq%20a_{max})
